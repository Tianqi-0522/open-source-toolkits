...
if __name__ == '__main__':
    ...
    cam = cv2.VideoCapture('D:\\download\\lensless_system\\selected_videos\\44_blood2_good.avi')  # 读取视频
    ...
    while True:
        ...
        if ret == True:  # 判断视频是否结束
            ...
            flow = cv2.calcOpticalFlowFarneback(prevgray, gray, None, 0.5, 1, 10, 3, 5, 1.2, cv2.OPTFLOW_FARNEBACK_GAUSSIAN)  # Farnback光流法
            prevgray = gray  # 计算完光流后，将当前帧存储为下一次计算的前一帧

            # cv2.imshow('flow', draw_flow(gray, flow))
            HSV_result = cv2.resize(draw_hsv(flow), (960, 540))
            cv2.imwrite(str(cnt)+'_'+str(cnt+1)+".png", draw_hsv(flow))
            cnt+=1
            if show_hsv:
                cv2.imshow('flow HSV', HSV_result)
            if show_glitch:
